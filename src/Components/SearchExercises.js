...
function SearchExercises() {
  ...
  const handleSearchBtn = async () => {
    ...
    if(searched)
    {
        const exerciseData = await fetchData(`https://exercisedb.p.rapidapi.com/exercises/target/${searched}`,exerciseOptions)

        console.log(exerciseData.slice(0,5)
        )

        const searchedExercise = exerciseData.filter( 
            (exercise)=> exercise.name.toLowerCase().includes(searched)
  ...
  return (
    <div className="mb-8 mt-8">
      <h1 className="text-6xl text-black mt-2 mb-4">
      ...
      <Box>
        ...
        />
        <button
          className="border-radius rounded-sm  text-2xl  bg-red-500 mt-1 ml-4   px-8 py-2 text-white font-serif"
